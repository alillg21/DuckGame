...
public class ChallengeHistoryListItem extends BaseListItem {
	...
	public ChallengeHistoryListItem(final ComponentActivity componentActivity, final Configuration configuration,
			final Challenge challenge, final boolean showPrize) {
	...
	private void fillView(final ViewHolder holder, final Drawable drawable, final String contestantName,
			final String contestantScore, final String prize) {
		holder.icon.setImageDrawable(drawable != null ? drawable : getContext().getResources().getDrawable(
				R.drawable.sl_icon_challenges));
		holder.contenderName.setText(_challenge.getContender().getDisplayName());
		holder.contenderScore.setText(StringFormatter.formatScore(_challenge.getContenderScore(),
				getComponentActivity().getConfiguration()));
		holder.contestantName.setText(contestantName != null ? contestantName : _challenge.getContestant()
				.getDisplayName());
		holder.contestantScore.setText(contestantScore != null ? contestantScore : getContext().getResources()
				.getString(R.string.sl_pending));
	...
	protected void prepareView(final ViewHolder holder) {
		if (_challenge.isComplete()) {
			...
			fillView(holder, drawable, null, StringFormatter.formatScore(_challenge.getContestantScore(),
					getComponentActivity().getConfiguration()), sign + StringFormatter
					.formatMoney(new Money(prize), getComponentActivity().getConfiguration()));

		} else if (_challenge.isOpen()) {
			fillView(holder, null, getContext().getResources().getString(R.string.sl_anyone), getContext()
					.getResources().getString(R.string.sl_pending), null);
			...
			fillView(holder, null, null, getContext().getResources().getString(R.string.sl_rejected), StringFormatter
					.formatMoney(new Money(BigDecimal.ZERO), getComponentActivity().getConfiguration()));
