...
public class GameDetailListActivity extends ComponentListActivity<BaseListItem> {
	...
	private void addUsers(final BaseListAdapter<BaseListItem> adapter, final List<User> users) {
		if(users.size() == 0) {
			adapter.add(new NoBuddiesListItem(this, getString(R.string.sl_no_friends_playing)));
		}
		else for (final User user : users) {
