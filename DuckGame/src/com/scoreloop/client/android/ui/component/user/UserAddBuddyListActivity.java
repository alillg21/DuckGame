		...
		if (requestController == usersSearchController) {
			if (usersSearchController.isOverLimit()) {
				if (usersSearchController.isMaxUserCount()) {
					...
					showToast(String.format(getResources().getString(R.string.sl_format_found_many_users), usersSearchController
							.getCountOfUsers()));
				}
				return;
			}
			...
			if (users.isEmpty()) {
				...
				ManageBuddiesTask.addBuddies(users, getManager(), new ManageBuddiesContinuation() {
