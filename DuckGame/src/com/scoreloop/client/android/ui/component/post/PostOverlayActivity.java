...
public class PostOverlayActivity extends Activity implements RequestControllerObserver, OnCheckedChangeListener,
	...
	private static Map<String, Entity>	_messageTargetMap		= new HashMap<String, Entity>();
	...
	private static final String			EXTRA_IDENTIFIER		= "messageTargetIdentifier";

	public static void assignMessageTarget(final Intent intent, final Entity messageTarget) {
		final String identifier = "post-" + ++_counter;
		_messageTargetMap.put(identifier, messageTarget);
	...
	public static Entity retrieveAndUnassignMessageTarget(final Intent intent) {
		final String identifier = intent.getStringExtra(EXTRA_IDENTIFIER);
		final Entity result = _messageTargetMap.get(identifier);
		_messageTargetMap.remove(identifier);
		return result;
	}
	...
	protected Entity getMessageTarget() {
		if (_messageTarget == null) {
			_messageTarget = retrieveAndUnassignMessageTarget(getIntent());
		}
	...
	protected boolean installMessageTarget() {
		_messageTarget = getMessageTarget();
		if (_messageTarget == null) {
			if (_messageTarget.getIdentifier() == null) {
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		_postButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(final View v) {
				_messageController.setTarget(_messageTarget);
	...
	public void requestControllerDidReceiveResponse(final RequestController aRequestController) {
		if (aRequestController == _messageController) {
			...
			setPosted(getApplicationContext(), _messageTarget);
			finish();
		}
	}
	...
	public void socialProviderControllerDidCancel(final SocialProviderController controller) {
		blockUI(false);
		updateProviderCheckbox(controller);
	}
	...
	private void updateProviderCheckbox(final SocialProviderController controller) {
		final SocialProvider provider = controller.getSocialProvider();
		if (!provider.isUserConnected(Session.getCurrentSession().getUser())) {
