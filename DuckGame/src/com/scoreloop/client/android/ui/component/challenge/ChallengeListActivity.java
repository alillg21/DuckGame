...
public class ChallengeListActivity extends ComponentListActivity<BaseListItem> {
	...
	public void onListItemClick(final BaseListItem item) {
		...
		if (item == _expandableHistoryListItem) {
			...
			display(factory.createChallengeAcceptScreenDescription(((ChallengeOpenListItem) item).getChallenge()));
		} else if (item.getType() == Constant.LIST_ITEM_TYPE_CHALLENGE_NEW) {
			display(factory.createChallengeCreateScreenDescription(((ChallengeCreateListItem) item).getUser(), null));
	...
	private void updateListIfReady() {
		...
		if (_open.size() > 0) {
			...
			for (final Challenge challenge : _open) {
				...
				adapter.add(new ChallengeOpenListItem(this, getResources().getDrawable(R.drawable.sl_icon_user), challenge));
			}
		} else {
			adapter.add(new ChallengeSimpleListItem(this, getResources().getString(R.string.sl_no_open_challenges)));
		}
		...
		if (_history.size() > 0) {
			...
			for (final Challenge challenge : _history) {
				if (challenge.isComplete() || challenge.isOpen() || challenge.isAssigned() || challenge.isRejected()
					...
					adapter.add(new ChallengeHistoryListItem(this, configuration, challenge, _showPrize));
				}
			}
			if (!addedItem) {
				adapter.add(new ChallengeSimpleListItem(this, getResources().getString(R.string.sl_no_history_challenges)));
			}
		} else {
			adapter.add(new ChallengeSimpleListItem(this, getResources().getString(R.string.sl_no_history_challenges)));
		}
		...
		adapter.add(new ChallengeCreateListItem(this, getResources().getDrawable(R.drawable.sl_icon_challenge_anyone), null));
		for (final User user : _buddies) {
			adapter.add(new ChallengeCreateListItem(this, getResources().getDrawable(R.drawable.sl_icon_user), user));
		}
	}
}
