...
public class StringFormatter {
	...
	public static String getAchievementsSubTitle(final Context context, final ValueStore model, final boolean extendedText) {
		final Integer numAchieved = model.<Integer> getValue(Constant.NUMBER_ACHIEVEMENTS);
		final Integer numTotal = model.<Integer> getValue(Constant.NUMBER_AWARDS);
	...
	public static String getBalanceSubTitle(final Context context, final ValueStore model, final Configuration configuration) {
		final Money balance = model.<Money> getValue(Constant.USER_BALANCE);
		if (balance != null) {
	...
	public static String getBuddiesSubTitle(final Context context, final ValueStore model) {
		final Integer count = model.<Integer> getValue(Constant.NUMBER_BUDDIES);
		if (count != null) {
	...
	public static String getChallengesSubTitle(final Context context, final ValueStore model) {
		final Integer numWon = model.<Integer> getValue(Constant.NUMBER_CHALLENGES_WON);
		final Integer numTotal = model.<Integer> getValue(Constant.NUMBER_CHALLENGES_PLAYED);
	...
	public static String getGamesSubTitle(final Context context, final ValueStore model) {
		final Integer count = model.<Integer> getValue(Constant.NUMBER_GAMES);
		if (count != null) {
	...
	public static Drawable getNewsDrawable(final Context context, final ValueStore contentValues, final boolean large) {
		final List<RSSItem> feed = contentValues.getValue(Constant.NEWS_FEED);
		final Integer count = contentValues.getValue(Constant.NEWS_NUMBER_UNREAD_ITEMS);
		int id = 0;
	...
	public static String getNewsSubTitle(final Context context, final ValueStore model) {
		final Integer count = model.<Integer> getValue(Constant.NEWS_NUMBER_UNREAD_ITEMS);
		if (count != null) {
			if (count == 0) {
				final List<RSSItem> feed = model.getValue(Constant.NEWS_FEED);
	...
	public static String getScoreTitle(final Context context, final Score score) {
		...
		return String.format(context.getString(R.string.sl_format_score_title), rank, score.getUser().getDisplayName());
	}
}
